...
class TrainLoop(LightningModule):
    ...
    def forward_backward(self, batch, cond):
        ...
        def training_losses(model, x_start, t, model_kwargs=None):
            """
            ...
            if model_kwargs['dpm_cond_img'] is not None:
                output = model(th.cat((x_start, model_kwargs['dpm_cond_img']), dim=1).float(), t, **model_kwargs)
            else:
                output = model(x_start.float(), t, **model_kwargs)
    ...
    def log_sampling(self, batch, sampling_model):
        ...
        print(f"Inference with {sampling_model}...")
        ...
        t = th.zeros(n).to(dat.device).long()
        if model_kwargs['dpm_cond_img'] is not None:
            output = model(th.cat((x_start, model_kwargs['dpm_cond_img']), dim=1).float(), t, **model_kwargs)
        else:
            output = model(x_start.float(), t, **model_kwargs)
        ...
        ddim_sample_plot = ((model_output + 1) * 127.5) / 255.
        log_image_fn(key=f'{sampling_model} - output', image=make_grid(ddim_sample_plot, nrow=4), step=(step_ + 1) * self.n_gpus)
