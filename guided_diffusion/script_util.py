...
from guided_diffusion.models.controlnet_nothing.controlnet_nothing import ControlNetWrapper_nothing
...
from guided_diffusion.models.controlnet_no_xt_no_hint_block.controlnet_no_xt_no_hint_block import ControlNet_no_xt_no_hint_block, ControlledUnetModel_no_xt_no_hint_block, ControlNetWrapper_no_xt_no_hint_block
...
def create_img_and_diffusion(cfg, logger=None):
    ...
    if cfg.img_model.arch in ['ControlNet', 'ControlledUnetModel']:
        ...
        img_model = ControlNetWrapper(controlnet=controlnet, unet=controlled_unet, control_mode=cfg.control_net.control_mode)
        img_cond_model = None
        ...
        n_unet = count_trainable_params(img_model.unet)
        logger.warning(f"[#] Model size ({cfg.img_model.arch}): ")
        logger.info(controlled_unet_no_hint_block)
        logger.info(controlnet_no_hint_block)
        logger.info(f"1. ControlNet ({cfg.img_cond_model.arch}): {n_ctrl/1e6}M")
        logger.info(f"2. UNet ({cfg.img_model.arch}): {n_unet/1e6}M")
        logger.warning(f"=> Total params: {(n_ctrl+n_unet)/1e6}M")
        logger.warning(f"[#] Control mode: {cfg.control_net.control_mode}")
    ...
    elif cfg.img_model.arch in ['ControlNet_nothing', 'ControlledUnetModel_nothing']:
        controlled_unet_nothing = create_model(cfg.img_model, all_cfg=cfg)
        controlnet_nothing = create_model(cfg.img_cond_model, all_cfg=cfg)
        img_model = ControlNetWrapper_nothing(controlnet=controlnet_nothing, unet=controlled_unet_nothing, control_mode=cfg.control_net.control_mode)
        img_cond_model = None
        n_ctrl = count_trainable_params(img_model.controlnet)
        n_unet = count_trainable_params(img_model.unet)
        logger.warning(f"[#] Model size ({cfg.img_model.arch}): ")
        logger.info(controlled_unet_nothing)
        logger.info(controlnet_nothing)
        logger.info(f"1. ControlNet ({cfg.img_cond_model.arch}): {n_ctrl/1e6}M")
        logger.info(f"2. UNet ({cfg.img_model.arch}): {n_unet/1e6}M")
        logger.warning(f"=> Total params: {(n_ctrl+n_unet)/1e6}M")
        logger.warning(f"[#] Control mode: {cfg.control_net.control_mode}")
...
def create_model(cfg, all_cfg=None):
    ...
    elif cfg.arch == 'ControlNet_no_hint_block':
        return ControlNet_no_hint_block(
            image_size=cfg.image_size,
            in_channels=3,
            model_channels=cfg.num_channels,
            hint_channels=cfg.in_channels,
    ...
    elif cfg.arch == 'ControlledUnetModel_no_hint_block':
        return ControlledUnetModel_no_hint_block(
            image_size=cfg.image_size,
            in_channels=cfg.in_channels,
            model_channels=cfg.num_channels,
            out_channels=cfg.out_channels,
    ...
    elif cfg.arch == 'ControlNet_nothing':
        return ControlNet_nothing(
            image_size=cfg.image_size,
            in_channels=3,
            model_channels=cfg.num_channels,
            hint_channels=cfg.in_channels,
    ...
    elif cfg.arch == 'ControlledUnetModel_nothing':
        return ControlledUnetModel_nothing(
            image_size=cfg.image_size,
            in_channels=cfg.in_channels,
            model_channels=cfg.num_channels,
            out_channels=cfg.out_channels,
    ...
    elif cfg.arch == 'ControlNet_nothing_no_decoder':
        return ControlNet_nothing_no_decoder(
            image_size=cfg.image_size,
            in_channels=3,
            model_channels=cfg.num_channels,
            hint_channels=cfg.in_channels,
