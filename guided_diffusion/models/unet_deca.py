...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        resblock_module = ResBlock if not self.condition else ResBlockCondition
        time_embed_seq_module = TimestepEmbedSequential if not self.condition else TimestepEmbedSequentialCond 
...
class UNetModelCondition(UNetModel):
    def __init__(self, image_size, in_channels, model_channels, out_channels, num_res_blocks, attention_resolutions, dropout=0, channel_mult=(1, 2, 4, 8), conv_resample=True, dims=2, num_classes=None, use_checkpoint=False, use_fp16=False, num_heads=1, num_head_channels=-1, num_heads_upsample=-1, condition_dim=159, use_scale_shift_norm=False, resblock_updown=False, use_new_attention_order=False, condition=True):
        super().__init__(image_size, in_channels, model_channels, out_channels, num_res_blocks, attention_resolutions, condition, dropout, channel_mult, conv_resample, dims, num_classes, use_checkpoint, use_fp16, num_heads, num_head_channels, num_heads_upsample, use_scale_shift_norm, resblock_updown, use_new_attention_order)
