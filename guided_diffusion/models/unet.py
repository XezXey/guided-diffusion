...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.block_hw = [int(image_size/(2**i)) for i in range(len(self.channel_mult))]
        self.middle_block_hw = self.block_hw[-1]
        print(self.block_hw, self.middle_block_hw, self.block_hw[::-1])
        ...
        self.middle_block = time_embed_seq_module(
            resblock_module(
                ...
                use_scale_shift_norm=use_scale_shift_norm,
                condition_dim=condition_dim,
                image_size=self.middle_block_hw
            ),
            AttentionBlock(
                ch,
                use_checkpoint=use_checkpoint,
...
class UNetModelChnMem(nn.Module):
    def __init__(self, 
        image_size, 
        in_channels, 
        model_channels, 
...
class ResBlockChnMem(TimestepBlock):
    ...
    def __init__(
        ...
        h = w = self.image_size
        if add_mem:
            self.channels_mem = nn.Parameter(data=th.zeros(size=(1, self.n_channels_mem, h, w)))
    ...
    def _forward(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
        return self.skip_connection(x_orig) + h

def conv_outsize(src_size, p, k, s=1, d=1):
    '''
    :param p: padding
    :param k: kernel size
    :param s: stride
    :param d: dilation
    '''
    dst_size = ((src_size + (2 * p) - d * (k-1) - 1) / s) + 1
    return int(dst_size)