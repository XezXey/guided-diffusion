...
class AttentionBlockNormals(nn.Module):
    """
    An attention block that allows spatial positions to attend to each other.
...
class EncoderUNetModelNoTime(nn.Module):
    """
...
class UNetModel_SpatialCondition_Hadamart_Both(nn.Module):
    """
    ...
    def forward(self, x, timesteps, y=None, **kwargs):
        ...
        for i, module in enumerate(self.input_blocks[1:]):
            ...
            print("Before hadamart: ", h.shape)
            h = hadamart_layer(x=h, y=spatial_latent[0], apply_=apply_cond_layer[0])
            print("After hadamart: ", h.shape)
            print("Spatial latent shape: ", spatial_latent[0].shape)
            spatial_latent.pop(0)
            apply_cond_layer.pop(0)
            # The rest layer
            print("[#] h to rest layer: ", h.shape)
        ...
        print("[#] Pre-middle block...")
        print("[#] h to hadamart pre-middle block: ", h.shape)
        print("[#] Spatial latent shape: ", spatial_latent[0].shape)
        h = hadamart_pre_mb(x=h, y=spatial_latent[0], apply_=apply_cond_layer[0])
        print("[#] h to after hadamart pre-middle block: ", h.shape)
        spatial_latent.pop(0)
        apply_cond_layer.pop(0)
        ...
        print("[#] Post-middle block...")
        print("[#] h to hadamart post-middle block: ", h.shape)
        print("[#] Spatial latent shape: ", spatial_latent[0].shape)
        h = hadamart_post_mb(x=h, y=spatial_latent[0], apply_=apply_cond_layer[0])
        print("[#] h to after hadamart post-middle block: ", h.shape)
        spatial_latent.pop(0)
        apply_cond_layer.pop(0)
        ...
        for i, module in enumerate(self.output_blocks):
            print("#"*100)
            print("[#] Module: ", i)
            print("[#] h to output block: ", h.shape)
            h = th.cat([h, hs.pop()], dim=1)
            print("[#] h to after cat (dim=1): ", h.shape)
            h = module(h, emb, condition=kwargs)
            print("[#] h to after module: ", h.shape)
