...
class ControlledUnetModel_nothing_no_decoder_no_zeroconv(UNetModel):
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, control_mode='add', **kwargs):
        ...
        control = control[::-1] # Inverse to be able to use .pop()
        ...
        assert len(hs) == 0, f"[#] Skip connections has {len(hs)} layers left, but should be 0"
        h = h.type(x.dtype)
        return {'output':self.out(h)}

class ControlNet_nothing_no_decoder_no_zeroconv(nn.Module):
    def __init__(
            self,
            image_size,
            in_channels,
    ...
    def forward(self, x, timesteps, **kwargs):
        ...
        context = kwargs['kwargs']['cond_params'].type_as(x)
        context = None  # Force to not use the non-spatial condition
        assert context is None, '[#] ControlNet_no_nonspa should not use the non-spatial condition'
...
class ControlNetWrapper_nothing_no_decoder_no_zeroconv(nn.Module):
    def __init__(self, controlnet: ControlledUnetModel_nothing_no_decoder_no_zeroconv, unet: ControlledUnetModel_nothing_no_decoder_no_zeroconv, control_mode='add'):
        super().__init__()
