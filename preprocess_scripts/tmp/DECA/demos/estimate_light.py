...
def main(args):
    ...
    # fo = open("CelebAMask-HQ-light-anno.txt", "w")
    fo = open("ffhq-valid-light-anno.txt", "w")
    for i in tqdm(range(len(testdata))):
        ...
        a = opdict["light"].cpu().numpy().flatten()
        fo.write(name + ".jpg ")
        fo.write(" ".join([str(x) for x in a]) + "\n")
        # torch.save(opdict["light"], name + "_light.pt")
        # tmp = torch.load(name + "_light.pt")
        # print(tmp)
        # if i == 10:
          # break
