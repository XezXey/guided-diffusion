...
class DECA(nn.Module):
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
                render_orig=False, original_image=None, tform=None, use_template=False, mean_cam=None):
        images = codedict['images']
        batch_size = images.shape[0]
        
        ## decode
        if use_template:
            verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape']*0., expression_params=codedict['exp']*0., pose_params=codedict['pose']*0.)
        else:
            verts, landmarks2d, landmarks3d = self.flame(shape_params=codedict['shape'], expression_params=codedict['exp'], pose_params=codedict['pose'])
        if self.cfg.model.use_tex:
            albedo = self.flametex(codedict['tex'])
            # save_folder = 'TestSamples/examples/albedo/'
            # os.makedirs(save_folder, exist_ok=True)
            # albedo = self.flametex(codedict['tex'])
            # plt.imshow(np.transpose(albedo[0].cpu().numpy(), (1, 2, 0)))
            # plt.title('albedo_original')
            # plt.savefig(f"{save_folder}/albedo_original_{codedict['name']}.png")
            # mask = imread(self.cfg.model.face_mask_path).astype(np.float32)/255.; mask = torch.from_numpy(mask[:,:,0])[None,None,:,:].contiguous().to(self.device)
            # plt.imshow(np.transpose((albedo * mask)[0].cpu().numpy(), (1, 2, 0)))
            # plt.title('albedo_face')
            # plt.savefig(f"{save_folder}/albedo_face_{codedict['name']}.png")
            # albedo_avg = ((mask>0) * albedo).sum(dim=(0, 2, 3))/(mask>0).sum(dim=(0, 2, 3))
            # # print(albedo_avg.shape)
            # albedo = torch.ones_like(albedo)
            # albedo[0, 0, :, :] *= albedo_avg[0]
            # albedo[0, 1, :, :] *= albedo_avg[1]
            # albedo[0, 2, :, :] *= albedo_avg[2]
            # # print(np.unique(mask.cpu().numpy()))
            # # print(mask.shape)
            # plt.imshow(np.transpose((mask * albedo)[0].cpu().numpy(), (1, 2, 0)))
            # plt.title('albedo_avg')
            # plt.savefig(f"{save_folder}/albedo_avg_{codedict['name']}.png")
        else:
            albedo = torch.zeros([batch_size, 3, self.uv_size, self.uv_size], device=images.device) 
        landmarks3d_world = landmarks3d.clone()

        ## projection
        if mean_cam is not None:
            cam = mean_cam
            assert cam.shape == codedict['cam'].shape
        else:
            cam = codedict['cam']
        ...
        if rendering:
            ...
            # ops = self.render(verts, trans_verts, albedo, h=h, w=w, background=background, lights=codedict['light'])
            ops = self.render(verts, trans_verts, albedo, h=h, w=w, lights=codedict['light'])
            ## output
        ...
        if return_vis:
            ## render shape
            shape_images, _, grid, alpha_images, albedo_images = self.render.render_shape(verts, trans_verts, h=h, w=w, return_grid=True, lights=codedict['light'])
            # shape_images, _, grid, alpha_images, albedo_images = self.render.render_shape(verts, trans_verts, h=h, w=w, return_grid=True)
            # shape_images, _, grid, alpha_images = self.render.render_shape(verts, trans_verts, h=h, w=w, images=background, return_grid=True, lights=codedict['light'])
            if use_detail:
                detail_normal_images = F.grid_sample(uv_detail_normals, grid, align_corners=False)*alpha_images
                shape_detail_images = self.render.render_shape(verts, trans_verts, detail_normal_images=detail_normal_images, h=h, w=w, lights=codedict['light'])
            ...
            if self.cfg.model.use_tex:
                ...
                visdict['rendered_images_pred_detail'] = predicted_detail_images
                visdict['rendered_images_uv_texture'] = ops_gt['images']
