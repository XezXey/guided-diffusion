...
if __name__ == '__main__':
    ...
    for _ in t:
        ...
        rendered_image, orig_visdict = params_utils.render_deca(deca_params=model_kwargs, 
                                                                ...
                                                                deca_obj=deca)
        text += "[#] Elapsed time ({}) => render : {:.3f}".format(model_kwargs['image_name'][0], time.time() - s_ren_t)
        ...
        s_ray_t = time.time()
        shadow_mask = gen_shadow_mask(ld, th.tensor(depth_grid.reshape(h, w, 3)))
        text += ", 2.raycasting : {:.3f}".format(time.time() - s_ray_t)
        # print("[#] SHADOW_MASK's shape : ", shadow_mask.shape)
        torchvision.utils.save_image(shadow_mask, f"./output/{model_kwargs['image_name'][0]}.png")
