...
-r /data/mint/sampling/Videos_fulllight_single/log=Masked_Face_woclip+BgNoHead+shadow_256_cfg=Masked_Face_woclip+BgNoHead+shadow_256.yaml/ema_085000/valid/reverse_sampling/src="$s"/"$dst"/diff=1000/ \
...
"
         ...
         cat $(find . -wholename "./videos/$s/$s$dst/compare_rmv_border/frame*.png" -print | sort -V | tail -n +7) | ffmpeg -i - -y -c:v libx264 -pix_fmt yuv420p -crf 18 ./final_videos/"$s"/"$s""$dst".mp4
    done
done
