...
do
    ...
    #for dst in $(ls -a /data/mint/sampling/Videos_fulllight_single/log=Masked_Face_woclip+BgNoHead+shadow_256_cfg=Masked_Face_woclip+BgNoHead+shadow_256.yaml/ema_085000/valid/reverse_sampling/src="$s"_smooth/ | grep -v "^\.\{1,2\}$")
    for dst in $(ls -a /data/mint/sampling/Videos_fulllight_single/log=Masked_Face_woclip+BgNoHead+shadow_256_cfg=Masked_Face_woclip+BgNoHead+shadow_256.yaml/ema_085000/valid/reverse_sampling/src="$s"/ | grep -v "^\.\{1,2\}$")
    do
        echo "[!] Running : "$s $dst
        echo -e "\n"
        echo "
            python unalign_parallel.py \
-i /data/mint/DPM_Dataset/Videos/"$s"_smooth/images/ \
-o /data/mint/videos_realigned/"$s"_smooth/"$s"_smooth"$dst"/aligned/ \
-r /data/mint/sampling/Videos_fulllight_single/log=Masked_Face_woclip+BgNoHead+shadow_256_cfg=Masked_Face_woclip+BgNoHead+shadow_256.yaml/ema_085000/valid/reverse_sampling/src="$s"_smooth/"$dst"/diff=1000/ \
-c ./videos/"$s"_smooth/"$s"_smooth"$dst"/composite/ \
-cmp ./videos/"$s"_smooth/"$s"_smooth"$dst"/compare_rmv_border/ \
-chsize "$chsize" \
-ap_dir /data/mint/DPM_Dataset/Videos/"$s"_smooth/"$s"_align_params.npy
"
        # python unalign_parallel.py \
        ...
        #python unalign_parallel.py \
            ...
            #-o /data/mint/videos_realigned/"$s"_smooth/"$s"_smooth"$dst"/aligned/ \
            #-r /data/mint/sampling/Videos_fulllight_single/log=Masked_Face_woclip+BgNoHead+shadow_256_cfg=Masked_Face_woclip+BgNoHead+shadow_256.yaml/ema_085000/valid/reverse_sampling/src="$s"_smooth/"$dst"/diff=1000/ \
            #-c ./videos/"$s"/"$s"_smooth"$dst"/composite/ \
            #-cmp ./videos/"$s"/"$s"_smooth"$dst"/compare_rmv_border/ \
            #-chsize "$chsize" \
            #-ap_dir /data/mint/DPM_Dataset/Videos/"$s"_smooth/"$s"_align_params.npy
        echo "[!] Done : "$s

        cat $(find . -wholename "./videos/$s/$s$dst/compare_rmv_border/frame*.png" -print | sort -V | tail -n +0) | ffmpeg -i - -y -c:v libx264 -pix_fmt yuv420p -crf 18 ./final_videos/"$s"/"$s""$dst".mp4
