...
def image_align(src_file,
    ...
    # print(relit_file)
    # print(dst_file)
    # print(composite_file)
    # print(compare_file)
    # exit()
    os.makedirs(f'./own_blending/{sj_name}/{dst_name}/imgs', exist_ok=True)
    ...
    if load_align_params is None:
        ...
        quad = load_align_params[name]['quad']
        pad = load_align_params[name]['pad']
        do_pad = load_align_params[name]['do_pad']
        crop = load_align_params[name]['crop']
        do_crop = load_align_params[name]['do_crop']
        shrink = load_align_params[name]['shrink']
        do_shrink = load_align_params[name]['do_shrink']
    # Transform.
    # print(quad)
    # print(quad.shape)
    ...
    inv_transformed.save(f"./own_blending/{sj_name}/{dst_name}/imgs/inverse_transf_{composite_file.split('/')[-1]}", 'PNG')
...
def work_landmark(raw_img_path, img_name, face_landmarks, load_align_params):
    ...
    match_src = re.search(r"src=([^/]+)", RELIT_IMAGES_DIR)
    if match_src:
        sj_name = match_src.group(1)
    else:
        sj_name = ''
        assert False 
        
    match_dst = re.search(r"light=\d+", RELIT_IMAGES_DIR)

    if match_dst:
        dst_name = match_dst.group(0)
    else:
        dst_name = ''
        assert False 
    ...
    image_align(raw_img_path,
                ...
                compare_face_path,
                face_landmarks,
                load_align_params=load_align_params,
                output_size=256,
                sj_name=sj_name,
                dst_name=dst_name)
...
if __name__ == "__main__":
    ...
    ALIGN_PARAMS_DIR = args.align_params_path
    
    if ALIGN_PARAMS_DIR is None:
        load_align_params = None
    elif os.path.exists(ALIGN_PARAMS_DIR):
        print("[#] UnAlign images with parameters...", ALIGN_PARAMS_DIR)
        load_align_params = np.load(ALIGN_PARAMS_DIR, allow_pickle=True).item()
    ...
    for img_name in tqdm(files):
        ...
        raw_img_path_list.append(raw_img_path)
        img_name_list.append(img_name)
     
    with multiprocessing.Pool() as pool:
        # inputs = list(zip(raw_img_path_list, img_name_list, [None] * len(raw_img_path_list)))
        inputs = list(zip(raw_img_path_list, img_name_list, [None] * len(raw_img_path_list), [load_align_params]*len(raw_img_path_list)))
        ...
        for ic in tqdm(input_chunks):
            results = pool.starmap(work_landmark, ic)
        # work_landmark(raw_img_path_list, img_name_list, [None] * len(raw_img_path_list), [load_align_params]*len(raw_img_path_list))
    ...
    #                 start=1):
    #             work_landmark(raw_img_path, img_name, face_landmarks, load_align_params)
    #     else:
    #         face_landmarks = None
    #         work_landmark(raw_img_path, img_name, face_landmarks, load_align_params)
            # assert i == 1, f'{i}'
