...
def image_align(src_file,
    ...
    PIL.Image.fromarray(np.clip(blended_out*255.0, 0, 255).astype(np.uint8)).save(composite_file, 'PNG')
...
def work_landmark(raw_img_path, img_name, face_landmarks):
    ...
    match_dst = re.search(r"dst=\d+", RELIT_IMAGES_DIR)
...
if __name__ == "__main__":
    ...
    import re
    match = re.search(r"src=([^/]+)", RELIT_IMAGES_DIR)
    ...
    raw_img_path = os.path.join(RAW_IMAGES_DIR, src_name.replace('.png', '.jpg'))
    for i, face_landmarks in enumerate(
            landmarks_detector.get_landmarks(raw_img_path),
            start=1):
        if i == 1: face_landmarks = face_landmarks
    ...
    # print(img_name_list, raw_img_path, raw_img_path_list)
    # exit()
    import multiprocessing
    with multiprocessing.Pool() as pool:
        # inputs = list(zip(raw_img_path_list, img_name_list, [None] * len(raw_img_path_list)))
        inputs = list(zip(raw_img_path_list, img_name_list, [face_landmarks]*len(raw_img_path_list)))
        ...
        # print(RAW_IMAGES_DIR, raw_img_path)
        # exit()
