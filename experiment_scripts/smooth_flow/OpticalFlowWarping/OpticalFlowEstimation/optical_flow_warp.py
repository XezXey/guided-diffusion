...
parser.add_argument('--output_random_savepath', dest='output_random_savepath', help='Save path of random eval images', required=True)
...
if __name__ == "__main__":
  ...
  if args.mode == 'auto':
    ...
    while True:
      ...
      plt.imshow(warping_result)
      plt.title("Warping forward/backward between {} and {} \n(params={})\nl2_forward={}, l2_backward={}".format(i, j, config_opticalflow, l2_forward, l2_backward))
      plt.show()

  elif mode == 'random':
      # This loop iterate until finishins all of pixles.
      # Read the <video_name>_random_generated_frame.ini for each video
      config = configparser.ConfigParser()
      config.read(args.image_dir + '/random_eval.ini')
      eval_step_list = config['IMAGES_DIR']['random_eval_step'].split('\n')
      n_samples = int(config['IMAGES_DIR']['n_samples'])
      for idx in tqdm(range(n_samples), desc=video_name):
        results_dict = {'params': [], 'l2_forward':[], 'l2_backward':[]}
        i = int(eval_step_list[idx].split(',')[0])
        j = i + int(eval_step_list[idx].split(',')[1])
        # Make sure that i, j is not out of bound
        if (i<0) or (j<0):
          exit()
        if (j>=len(image_filelist)):
          j=len(image_filelist)-1
        # Create a eval_savepath
        output_eval_savepath = args.output_random_savepath + 'eval_results/warped_{}<->{}'.format(i, j)
        if not os.path.exists(output_eval_savepath):
          os.makedirs(output_eval_savepath)
        ...
        plt.imshow(warping_result)
        plt.title("Warping forward/backward between {} and {} \n(params={})\nl2_forward={}, l2_backward={}".format(i, j, config_opticalflow, l2_forward, l2_backward))
        plt.savefig(output_eval_savepath + '/{}.png'.format(config_opticalflow))

        results_dict['params'].append(config_opticalflow)
        results_dict['l2_forward'].append(l2_forward)
        results_dict['l2_backward'].append(l2_backward)

        # Save results to excel
        if os.path.isfile(output_eval_savepath + '/warped_{}<->{}_results.xlsx'.format(i, j)):
          results_df = pd.read_excel(output_eval_savepath + '/warped_{}<->{}_results.xlsx'.format(i, j))
          results_df = results_df.append(pd.DataFrame(results_dict)).loc[:, results_dict.keys()].to_excel(output_eval_savepath + '/warped_{}<->{}_results.xlsx'.format(i, j))
        else:
          pd.DataFrame(results_dict).to_excel(output_eval_savepath + '/warped_{}<->{}_results.xlsx'.format(i, j))
