...
parser.add_argument('--sh_grid_size', type=int, default=3, help='grid size of SH')
args = parser.parse_args()

'''
# Command
python relight_grid_sh.py --dataset ffhq --set valid --step 050000 --out_dir /data/mint/sampling/changing_bg/rotateSH 
--cfg_name difareli_nobg_128.yaml --log_dir difareli_nobg_128 
--diffusion_steps 1000 --timestep_respacing 250 --seed 47 
--sample_pair_json ./sample_json/relight_bg/hoodie.json --sample_pair_mode pair 
--itp render_face --itp_step 9 --batch_size 2 --gpu_id 3 --lerp --save_vid --idx 0 1 --sh_grid_size 3 --sh_span_x -4 4 --sh_span_y 4 -4 --use_sh --sh_scale 0.6
'''
    ...
    for ckpt in args.ckpt_step:
        for time_respace in args.time_respace:
            for sample_json in args.sample_pair_json:
                ...
                cmd = (
                    f"""
                    python relight_gridSH.py --ckpt_selector {args.ckpt_type} --dataset ffhq --set valid --step {ckpt} --out_dir {args.out_dir} \
                    ...
                    --postfix step={time_respace} --save_vid --sh_grid_size {args.sh_grid_size} --sh_span_x -4 4 --sh_span_y 4 -4 --use_sh --sh_scale 0.6"""
                    )
                print(cmd)
                os.system(cmd)
                print("#"*100)