...
if __name__ == '__main__':
    ...
    # Modified the canny value here
    if args.canny_thres is not None:
        print(f"[#] Modify the canny threshold... from {cfg.img_cond_model.canny_thres}", end='')
        cfg.img_cond_model.canny_thres[cfg.img_cond_model.in_image.index('canny_edge_bg')] = args.canny_thres
        print(f" to {cfg.img_cond_model.canny_thres}...Done!")
    ...
    for i in range(start, end):
        ...
        is_canny = True if ((out_render is not None) and ('canny_edge_bg') in cfg.img_cond_model.in_image) else False
        if is_canny:
            vis_utils.save_images(path=f"{save_res_dir}", fn="canny", frames=out_render[:, 3:4].mul(255).add_(0.5).clamp_(0, 255)/255.0)
