...
def relight_with_solver(dat, model_kwargs, itp_func, n_step=3, src_idx=0, dst_idx=1):
    ...
    model_dict[model_kwargs['cfg'].img_model.name].return_dict = False
    model_fn = model_wrapper(model_dict[model_kwargs['cfg'].img_model.name], 
                             noise_schedule,
                             model_type="noise",
    ...
    if solver_alg == 'dpmsolver' or solver_alg == 'dpmsolver++':
        ...
        solver = UniPC(model_fn, noise_schedule, algorithm_type='noise_prediction', verbose=False)
    ...
    for i in range(len(sub_step)-1):
        ...
        # Record the time
        sub_relight_time = time.time() - sub_start_relight_time   # Update the time per frame
    ...
    out_timing = {'rev_time':reverse_time, 'each_relit_time':relit_time, 'relit_time':np.mean(relit_time), 'render_time':cond['render_time'] if 'render_time' in cond else None}
...
if __name__ == '__main__':
    ...
    counter_sj = 0
    runtime_dict = {'rev_time':[], 'relit_time':[], 'sub_relit_time':[], 'render_time':[]}
    for i in range(start, end):
        ...
        out_relit, out_cond, time_dict, misc_dict = relight_with_solver(dat = dat,
