...
class Diffusion_DECA(GaussianDiffusion):
    ...
    def __init__(self, img_model, deca_model, diffusion, progress=False, **kwargs):
    ...
    def p_sample_loop(self, shape_dict=None, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None):
        if noise is not None:
            ...
            img = th.randn(*shape_dict['img']).cuda()
        ...
        for i in indices:
            with th.no_grad():
                # Forward pass - img model
                t = th.tensor([i] * shape_dict['img'][0]).cuda()    # [i] * batch_size
                B, C = img.shape[:2]
                assert t.shape == (B,)
