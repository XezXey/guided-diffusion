...
def relight(dat, model_kwargs, itp_func, n_step=3, src_idx=0, dst_idx=1):
    ...
    for i in range(len(sub_step)-1):
        ...
        x_start = th.repeat_interleave(input=dat[0:1, ...], repeats=args.batch_size, dim=0).cuda()
        t = th.zeros(sub_step[i]).cuda().long()
        if model_kwargs['dpm_cond_img'] is not None:
            output = model(th.cat((x_start, model_kwargs['dpm_cond_img']), dim=1).float(), t, **cond_relit)
        else:
            output = model(x_start.float(), t, **cond_relit)
            
        model_output = output['output']
        relit_out.append(model_output.detach().cpu().numpy())
