...
parser.add_argument('--postproc_shadow_mask_smooth_diffuse_to_shadow', action='store_true', default=False)
...
from sample_utils import inference_utils
...
def make_condition(cond, src_idx, dst_idx, n_step=2, itp_func=None):
    ...
    cond, _, misc = inference_utils.build_condition_image(cond=cond, misc=misc, force_render=args.force_render)
    ...
    cond, shadow_weight = inference_utils.shadow_diff_with_weight_postproc(cond=cond, misc=misc_tmp)
    cond, misc = inference_utils.shadow_diff_final_postproc(cond=cond, misc=misc)
    if args.shadow_diff_blurmap:
        cond, misc = inference_utils.blur_map(cond=cond, misc=misc)
    n_step = misc['n_step']
    # Return the ['cond_img'] and ['dpm_cond_img']
    cond.update(inference_utils.prepare_cond_sampling(cond=cond, cfg=cfg, use_render_itp=True))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'itw':
        ...
        cfg.dataset.data_dir = f'{cfg.dataset.root_path}/{cfg.dataset.training_data}/images_aligned/'
        cfg.dataset.face_segment_dir = f"{cfg.dataset.root_path}/{cfg.dataset.training_data}/face_segment_with_pupil/"
    elif args.dataset == 'hard_cs':
        cfg.dataset.root_path = f'/data/mint/DPM_Dataset/'
        img_dataset_path = f"/data/mint/DPM_Dataset/Hard_CS/images_aligned/"
        deca_dataset_path = f"/data/mint/DPM_Dataset/Hard_CS/params/"
        img_ext = '.png'
    ...
    with open(f'{args.out_dir}/log={log_dir}_cfg={cfg_name}{args.postfix}/runtime.json', 'w') as fj:
        runtime_dict['name'] = f"log={log_dir}_cfg={cfg_name}{args.postfix}"
