...
parser.add_argument('--cfg_dir', type=str, required=True, help='path to config file.')
...
parser.add_argument('--timestep_respacing', nargs='+', type=str, default=[1000], help='timestep respacing')
# Cast shadows rendering
...
# Relighting
parser.add_argument('--interp_sh', action='store_true', default=False)
...
assert args.interp_sh + args.rotate_sh <= 1, "[#] Only one of --interp_sh, --rotate_sh can be set."
...
for ts in args.timestep_respacing:
                    ...
                    for scale_depth in args.scale_depth:
                        for cval in args.c:
                            for adj_contrast in adjust_contrast:
                                logger.warning("#"*100)
                                logger.info(f'[#] Running checkpoint {args.ckpt_type}_{ckpt}...')
                                logger.info(f'[#] Dataset: {dataset}')
                                logger.info(f'[#] Model dir: {args.model_dir}')
                                logger.info(f'[#] Cfg dir: {args.cfg_dir}')
                                logger.info(f'[#] Timestep respacing: {ts}')
                                ...
                                logger.info(f'[#] Interp SH: {args.interp_sh}')
                                ...
                                cmd = (
                                    ...
                                    --cfg_name {args.cfg_dir} --log_dir {args.model_dir} --timestep_respacing {ts} \
                                ...
                                if args.interp_sh: pass # No additional argument needed, default is interp sh
                                ...
                                else: cmd += f' --postfix SD{scale_depth}_{ts}T_{pf}'
