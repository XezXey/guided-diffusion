...
class DECA(nn.Module):
    def __init__(self, config=None, device='cuda', mode='deca', mask=None):
        ...
        if mode == 'only_renderer':
            self._create_flame(self.cfg.model)
            self._setup_renderer(self.cfg.model, mask=mask)
        else:
            self._create_model(self.cfg.model)
            self._setup_renderer(self.cfg.model, mask=mask)

    def _create_flame(self, model_cfg):
        self.flame = FLAME(model_cfg).to(self.device)
        self.flametex = FLAMETex(model_cfg).to(self.device)
        
    def _setup_renderer(self, model_cfg, mask=None):
        set_rasterizer(self.cfg.rasterizer_type)
        self.render = SRenderY(self.image_size, obj_filename=model_cfg.topology_path, uv_size=model_cfg.uv_size, rasterizer_type=self.cfg.rasterizer_type, mask=mask).to(self.device)
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if return_vis:
            ...
            # print(images.shape, uv_pverts.shape)
