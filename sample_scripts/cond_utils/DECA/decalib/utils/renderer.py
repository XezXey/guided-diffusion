...
class SRenderY(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d', mask=None):
    ...
    def render_shape(self, vertices, transformed_vertices, colors = None, images=None, detail_normal_images=None, 
                lights=None, light_type='point', return_grid=False, uv_detail_normals=None, h=None, w=None, R_normals=None, mask=None):
        '''
        ...
        if R_normals is not None:
            def rotate_normals(R, normal_images):
                b, c, h, w = normal_images.shape
                N = normal_images[:, :, None, :, :] # Make it BxCxHxW -> BxCx1xHxW
                R = R[..., None, None]
                N_rot = np.einsum('bijhw, bjkhw -> bikhw', R.cpu().numpy(), N.cpu().numpy())
                N_rot = torch.tensor(N_rot.reshape(b, c, h, w)).to(normal_images.device)
                return N_rot
            normal_images = rotate_normals(R_normals, normal_images)
