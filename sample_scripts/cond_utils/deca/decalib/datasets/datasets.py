...
class TestDataFromPath(Dataset):
...
class TestData(Dataset):
    def __init__(self, data_list, dat_type, iscrop=True, crop_size=224, scale=1.25, face_detector='fan', device='cuda'):
        '''
            testpath: folder, imagepath_list, image path, video path
        '''
        if dat_type == 'img':
            self.data = data_list 
            print('total {} images'.format(len(self.data['image'])))
        elif dat_type == 'path':
            if os.path.isdir(data_list): 
                self.data = glob(data_list + '/*.jpg') +  glob(data_list + '/*.png') + glob(data_list + '/*.bmp')
            elif os.path.isfile(data_list) and (data_list[-3:] in ['jpg', 'png', 'bmp']):
                self.data = [data_list]
            elif os.path.isfile(data_list) and (data_list[-3:] in ['mp4', 'csv', 'vid', 'ebm']):
                self.data = video2sequence(data_list)
                print(self.data)
            else:
                print(f'please check the test path: {data_list}')
                exit()
    ...
    def __len__(self):
        if self.dat_type == 'img':
            return len(self.data['image'])
        elif self.dat_type == 'path':
    ...
    def __getitem__(self, index):
        if self.dat_type == 'img':
            ...
            imagename = self.data['imagename'][index]
        elif self.dat_type == 'path':
