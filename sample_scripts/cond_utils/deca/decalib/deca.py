...
class DECA(nn.Module):
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True):
        ...
        # landmarks2d = landmarks2d.cpu().numpy()
        # landmarks3d = landmarks3d.cpu().numpy()
        # fig = make_subplots(rows=1, cols=2, specs=[[{'type':'scatter3d'}, {'type':'scatter3d'}]])
        # print(landmarks3d-landmarks2d)
        # fig.add_trace(go.Scatter3d(x=landmarks2d[..., 0].reshape(-1), y=landmarks2d[..., 1].reshape(-1), z=landmarks2d[..., 2].reshape(-1), mode='markers+lines'), row=1, col=1)
        # fig.add_trace(go.Scatter3d(x=landmarks2d[..., 0].reshape(-1), y=landmarks2d[..., 1].reshape(-1), z=landmarks2d[..., 2].reshape(-1), mode='markers+lines'), row=1, col=2)
        # fig.add_trace(go.Scatter3d(x=landmarks3d[..., 0].reshape(-1), y=landmarks3d[..., 1].reshape(-1), z=landmarks3d[..., 2].reshape(-1), mode='markers+lines'), row=1, col=2)
        # fig.show()
        # exit()
    ...
    def save_obj(self, filename, opdict):
        ...
        '''
        # colors=np.expand_dims(uvfaces, axis=0))
        i = 0
