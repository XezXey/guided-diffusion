...
def best_itp_method_vis(m_name, itp, ckpt):
  ...
  out += f"<h1>[#] Interpolate on : {itp} </h1>"
  folder = f"/home/mint/guided-diffusion/sample_scripts/py/relighting_sample_id/ddim_reverse_interpolate/samples/{m_name}/{ckpt}/"
...
def model_comparison(ema, itp_method):
  ...
  folder = f"/home/mint/guided-diffusion/sample_scripts/py/relighting_sample_id/ddim_reverse_interpolate/samples/log=cond_img64_by_deca_arcface_cfg=cond_img64_by_deca_arcface.yaml/ema_{ema}/valid/light/"
    ...
    for d in glob.glob(f"{folder}/{src_id}/dst=*"):
      ...
      for m in model:
        ...
        if m == "log=cond_img64_by_deca_arcface_cfg=cond_img64_by_deca_arcface.yaml":
          each_model = f"/home/mint/guided-diffusion/sample_scripts/py/relighting_sample_id/ddim_reverse_interpolate/samples/{m}/ema_{ema}/valid/light/"
        else:
          each_model = f"/home/mint/guided-diffusion/sample_scripts/py/relighting_sample_id/ddim_reverse_interpolate/samples/{m}/ema_{ema}/valid/spatial_latent/"
    ...
    for d in glob.glob(f"{folder}/{src_id}/dst=*"):
      ...
      for m_index, m in enumerate(model):
        ...
        if m == "log=cond_img64_by_deca_arcface_cfg=cond_img64_by_deca_arcface.yaml":
          ...
          each_model = f"/home/mint/guided-diffusion/sample_scripts/py/relighting_sample_id/ddim_reverse_interpolate/samples/{m}/ema_{ckpt_dict[m]}/valid/spatial_latent/"
        for d in glob.glob(f"{each_model}/{src_id}/dst=*"):
          img_path = glob.glob(d + f"/{itp_method}_1000/*.png")
          img_path = sort_by_frame(img_path)
          for f in img_path:
