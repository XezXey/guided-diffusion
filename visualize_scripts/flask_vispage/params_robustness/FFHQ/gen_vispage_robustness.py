        ...
        for s_id, src_dst in enumerate(subject_id):
            for param in ['shape', 'pose', 'exp', 'cam', 'all']:
                ...
                for to_show in ['res', 'ren']:
                    out += f"<tr>"
                    out += f"<th style=\"font-size:10px;white-space: nowrap;\"> {s_id+1}.({src_dst[0].split('.')[0]},{src_dst[1].split('.')[0]})<img src=/files/{data_path}/{src_dst[0].replace('png', 'jpg')} title=\"{src_dst[0]}\"><img src=/files/{data_path}/{src_dst[1].replace('png', 'jpg')} title=\"{src_dst[1]}\"> </th>"
