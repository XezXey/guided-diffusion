...
def create_app():
    ...
    def best_checkpoint_img_show(m_name, itp, itp_method, sampling, diff_step, n_frame=5, show='res'):
        ...
        for idx, src_dst in enumerate(subject_id):
            ...
            out += f"[#{idx+1}] {src_dst[0]} : <img src=/files/{data_path}/{src_dst[0]} width=\"64\" height=\"64\">, {src_dst[1]} : <img src=/files/{data_path}/{src_dst[1]} width=\"64\" height=\"64\">" + "<br>" + "<br>"
            for ckpt in checkpoint:
                out += "<tr>"
                ...
                out += f"<td> <img src=/files/{data_path}/{src_dst[0]}> </td>"
                out += "<td>"
                if len(frames) > 0:
    ...
    def model_comparison_from_json_lf(jf, itp_method, diff_step, sampling, ckpt, show):
        ...
        for s_id, src_dst in enumerate(subject_id):
            ...
            for m_id, m in enumerate(model):
                ...
                # if ('algined_images' not in m) and ('blur_images' not in m):
                #     src_dst[0] = src_dst[0].split('.')[0] + '.jpg'
                #     src_dst[1] = src_dst[1].split('.')[0] + '.jpg'
                    
                # else:
                #     src_dst[0] = src_dst[0].split('.')[0] + '.png'
                #     src_dst[1] = src_dst[1].split('.')[0] + '.png'
                bg_change = ['bd', 'rc', 'louve', 'black', 'orange', 'gray']
                ...
                if int(args.res) == 256:
                    ...
                    if 'original_images' in m:
                        each_model = f"{args.sample_dir}/{args.exp_dir}/{m}/{step}/{args.set_}/{itp}/{sampling}_sampling/src={src_dst[0][2:].replace('jpg', 'png')}/dst={src_dst[1].replace('.jpg', '.png')}/"
                    print(each_model)
                # else:
                #     each_model = f"{args.sample_dir}/{args.exp_dir}/{m}/{step}/{args.set_}/{itp}/{sampling}_sampling/src={src_dst[0].replace('png', 'jpg')}/dst={src_dst[1].replace('png', 'jpg')}/"
                # print("GGODDL")
...
if __name__ == "__main__":
    ...
    data_path = f"/home/mint/guided-diffusion/experiment_scripts/ITW_debugging/debugging_img/itw_images/replace_bg/rc_images/images/valid/"
