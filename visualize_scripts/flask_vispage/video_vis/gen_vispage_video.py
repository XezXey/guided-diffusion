...
sj_path = f"{args.sample_dir}/{args.exp_dir}/{model}/ema_085000/valid/reverse_sampling/"
data_path = "/data/mint/DPM_Dataset/Videos/"
...
def create_app():
    ...
    def subject_selector_show(sj_name):
        ...
        original = f"{data_path}/{sj_name}/aligned_images/valid/" 
        for id, lf in enumerate(light_from):
            lf_name = lf.split('/')[-1]
            out += "<table>"
            out += f"<tr> <th> Model; <pre> Image : src({sj_name}), dst({lf_name}) </pre> </th>"
            out += f"<tr>"
            frames = glob.glob(f"{lf}/diff=1000/res_f*")
            if len(frames) > 0:
                frames = sort_by_frame(frames)
                
                # Source
                out += "<tr> <td>"
                out += f"<img src=/files/data/mint/DPM_Dataset/ffhq_256_with_anno/ffhq_256/valid/{lf_name.split('=')[-1]}.jpg>"
                for f in frames:
                    f_tmp = f.split('/')[-1].split('_')[-1]
                    out += f"<img src=/files/{original}/{f_tmp}>"
                out += "</td> </tr>"
