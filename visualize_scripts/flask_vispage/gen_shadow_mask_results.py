...
def create_app():
    ...
    def root():
        #NOTE: Root path
        out = ""
        out += f"<a href=\"/shadow_mask_exp/\"> Shadow mask experiment (From json)</a> <br>"
        return out
    
    @app.route('/shadow_mask_exp/jf=<jf>&itp_method=<itp_method>&diff_step=<diff_step>&sampling=<sampling>&ckpt=<ckpt>&show=<show>/')
    def shadow(jf, itp_method, diff_step, sampling, ckpt, show):
        ...
        for s_id, src_dst in enumerate(subject_id):
            ...
            for m_id, m in enumerate(model):
                ...
                out += "<td>"
                # Videos
                vid_path = glob.glob(f"{each_model}/{itp_method}_{diff_step}/n_frames={n_frames}/{show}_rt.mp4")
                if len(vid_path) == 0:
    ...
    @app.route('/shadow_mask_exp/')
    def model_comparison_from_json_selector_shadow():
        out = ""
        json_files = glob.glob('./json_comparison/shadow_mask/*.json')
