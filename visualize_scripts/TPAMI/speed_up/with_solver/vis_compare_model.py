        ...
        for k, v in sample_pairs.items():
            ...
            for m_name, metadat in candidates.items():
                ...
                if 'solver' in alias:
                    ...
                    out += f"<td>Alg: {alias_str[0]} <br>Order: {alias_str[1]} <br>Method: {alias_str[2]} <br>Steps: {alias_str[3]} <br>Correction_x0_fn: {alias_str[4]} </td>"
                else: 
                ...
                if 'baseline' in alias:
                    ...
                    print(path)
                    frames = glob.glob(f"{path}/{itp_method}_diff={diff_step}_respace=/n_frames={n_frame}/res_*.png")
                else:
                    frames = glob.glob(f"{path}/{itp_method}_{diff_step}/n_frames={n_frame}/res_*.png")
                    # out += f"{path}/{itp_method}_{diff_step}/n_frames={n_frame}/res_*.png"
                # out += str(show_itmd)
