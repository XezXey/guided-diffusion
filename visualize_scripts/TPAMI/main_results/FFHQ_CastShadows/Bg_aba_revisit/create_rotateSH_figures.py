    ...
    def model_compare():
        ...
        for ts in to_show:
            ...
            if args.res == 128:
                ...
                out += f"{int(s) + count}.[#{k}] {src}=>{dst} : <img src=/files/{data_path}/{src.replace('jpg', 'png')}>, {dst} : <img src=/files/{data_path}/{dst.replace('jpg', 'png')}>" + ", Shadow area = " + f"<img height=\"128\" src=/files/{shadow_area_pth}/{args.set_}/{src.replace('jpg', 'png')}>" + "<br>" + "<br>"
            else:
                shadow_area_pth = '/data/mint/DPM_Dataset/ffhq_256_with_anno/shadow_diff_SS_with_c_simplified/vis/'
            ...
            for m_idx, metadat in candidates.items():
                ...
                if os.path.exists(f"{path}/{itp_method}_{diff_step}/n_frames={n_frame_tmp}/out_no1.mp4") and show_vid == "True":
                    out += f"""
                        <td>  
                        <video controls autoplay muted loop>
                            <source src=/files/{path}/{itp_method}_{diff_step}/n_frames={n_frame_tmp}/out_no1.mp4 type="video/mp4">
                        </video>
                        </td>
                    """
                else: 
