...
# parser.add_argument('--rank_shadow_c', action='store_false', default=True)
...
def create_app():
    ...
    def model_compare():
        ...
        data_path = f"/data/mint/DPM_Dataset/HoloRelighting/images_aligned/{args.set_}/"
        data_path_noaligned = f"/data/mint/DPM_Dataset/HoloRelighting/images/{args.set_}/"
        try:
        ...
        for ts in to_show:
            ...
            if args.res == 128:
                ...
                out += f"[#{k}] {src}=>{dst} : <img src=/files/{data_path}/{src.replace('jpg', 'png')}>, {dst} : <img src=/files/{data_path_noaligned}/{dst.replace('jpg', 'png')}>, {gt} : <img src=/files/{data_path_noaligned}/{dst.replace('jpg', 'png')}>" + ", Shadow area = " + f"<img height=\"128\" src=/files/{shadow_area_pth}/{args.set_}/{src.replace('jpg', 'png')}>" + "<br>" + "<br>"
            else:
                shadow_area_pth = '/data/mint/DPM_Dataset/HoloRelighting/shadow_diff_SS_with_c_simplified/vis/'
                out += f"[#{k}] {src}=>{dst} : <img src=/files/{data_path}/{src}>, {dst} : <img src=/files/{data_path_noaligned}/{dst}>, {gt} : <img src=/files/{data_path_noaligned}/{gt}>" + ", Shadow area = " + f"<img height=\"256\" src=/files/{shadow_area_pth}/{args.set_}/{src.replace('jpg', 'png')}>" + "<br>" + "<br>"
            # Model 
