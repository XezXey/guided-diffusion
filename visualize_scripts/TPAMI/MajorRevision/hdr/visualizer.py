...
def get_file_from_conv(path_conv, img_path, hdr, src, dst, n_frames=-1):
    if path_conv == "neural_gaffer":
        path = f"{img_path}/{src.split('.')[0]}/{hdr}/"
        print(path)
...
def create_app():
    ...
    def root():
        ...
        hdr = request.args.get('hdr', "012_hdrmaps_com_free_2K")
        ...
        for ts in to_show:
            ...
            for m_idx, metadat in candidates.items():
                # Model's metadata
                ckpt = metadat.get('step', 'Default')
                alias = metadat['alias']
                img_path = metadat['res_dir']
                n_frames = metadat.get('n_frames', -1)
