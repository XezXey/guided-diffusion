...
def create_app():
    ...
    def model_compare(sampling, show_itmd, show_recon, show_relit):
        ...
        for k, v in sample_pairs.items():
            out += "<table>"
            out += "<tr> <th> #N diffusion step </th> <th> Input </th> <th> Image </th> <th> Canny </th> </tr>"
            src = v['src']
            dst = v['dst']
            ...
            for m_name, metadat in candidates.items():
                ...
                canny_frames = glob.glob(f"{path}/{itp_method}_{diff_step}/n_frames={n_frame}/canny_f*.png")
                if len(canny_frames) > 0:
                    out += f"<td> <img src=/files/{canny_frames[-1]}> </td>"
                # if args.res == 128:
                #     # out += f"<td> <img src=/files/{data_path}/{src.replace('jpg', 'png')}> </td>"
                    # out += f"<td> <img src=/files/{canny_frames[-1]}> </td>"
                # else:
